## Главное меню: новая игра (имя + нация), продолжить, выход.
extends Control
...
func _ready() -> void:
	...
	subtitle.text = "Ветер свободы — открытый ремейк в духе «Корсаров II»"
	...
	_name_edit.placeholder_text = "Имя капитана"
	_name_edit.text = "Натаниэль"
	box.add_child(_name_edit)

	_nation_option = OptionButton.new()
	for n in ["england", "france", "spain", "holland", "pirates"]:
		_nation_ids.append(n)
		_nation_option.add_item("Служить: %s" % World.NATIONS[n]["name"])
	box.add_child(_nation_option)

	var new_btn := Button.new()
	new_btn.text = "Новая игра"
	new_btn.pressed.connect(_on_new_game)
	box.add_child(new_btn)

	var cont_btn := Button.new()
	cont_btn.text = "Продолжить"
	...
	var hint := Label.new()
	hint.text = "Море: WASD — паруса и руль, Q/E — залпы бортов,\nR — тип заряда, B — абордаж"
