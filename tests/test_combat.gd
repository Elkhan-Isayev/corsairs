...
func test_range_depends_on_ammo() -> void:
	...
	assert_gt(balls, bombs, "ядра летят дальше бомб")
	assert_gt(bombs, grape, "бомбы дальше картечи")
...
func test_hit_chance_bounds() -> void:
	...
	assert_eq(Combat.hit_chance(range_limit + 1.0, range_limit, 10, 10), 0.0, "вне дальности")
	var close := Combat.hit_chance(10.0, range_limit, 0, 0)
	var far := Combat.hit_chance(range_limit * 0.9, range_limit, 0, 0)
	assert_gt(close, far, "ближе — точнее")
	assert_lt(Combat.hit_chance(1.0, range_limit, 10, 10), 0.951, "шанс не выше 95%")
...
func test_broadside_deals_damage() -> void:
	...
	assert_eq(report["fired"], 12, "залп всем бортом (24/2)")
	assert_gt(report["hits"], 0, "в упор должны попадать")
	assert_lt(def.hull, hull_before, "корпус повреждён")
	assert_eq(att.reload_progress, 0.0, "после залпа идёт перезарядка")
...
func test_broadside_consumes_ammo() -> void:
	...
	assert_eq(report["fired"], 5, "стреляем не больше, чем есть ядер")
...
func test_no_fire_while_reloading() -> void:
	...
	assert_eq(second["fired"], 0, "во время перезарядки залпа нет")
...
func test_no_fire_out_of_range() -> void:
	...
	assert_eq(att.ammo_stock["balls"], 500, "боезапас не тратится впустую")
...
func test_knippels_shred_sails_not_hull() -> void:
	...
	# Несколько залпов для статистики.
	for i in 6:
	...
	assert_gt(sails_lost, hull_lost * 0.5, "книппели рвут паруса заметнее корпуса")
...
func test_grapeshot_kills_crew() -> void:
	...
	assert_lt(def.crew, crew_before, "картечь косит команду")
...
func test_reload_faster_with_skill_and_crew() -> void:
	...
	assert_lt(fast, slow, "навык ускоряет перезарядку")
	s.crew = int(s.spec()["max_crew"] * 0.3)
	assert_gt(Combat.reload_time(s, 0), slow, "неполная команда заряжает дольше")
...
func test_boarding_range() -> void:
	...
	def.hull = 0.0
	assert_false(Combat.can_board(50.0, def), "тонущего не абордируем")


func test_ammo_cycle() -> void:
	assert_eq(Ammo.next_type("balls"), "knippels")
	assert_eq(Ammo.next_type("bombs"), "balls", "цикл замыкается")
